...
def hdmb(chushi=None,fangxiang=None,queding=False,hdlx=None,gcwz=None):
    ...
    if gjid==sjk['主角']:
        ...
        if hdlx=='军事':
            wif.app.text('军队移动指令')
            if queding==True:
                sjk['AI'].zl(dqid=dqid,zl='移动军队',cs=[mbdqid,wif.appuifw.query(cn("移动兵力（0-%s）人"%(dqdx['兵力'])),'number')])
        elif hdlx=='粮食交易周边卖出':
            wif.app.text('粮食交易指令')
            if queding==True:
                sjk['AI'].zl(dqid=dqid,zl='粮食交易',cs=[mbdqid,wif.appuifw.query(cn("粮食交易周边卖出（0-%s）单位"%(dqdx['粮食']['国库'])),'number'),'周边卖出'])
        elif hdlx=='粮食交易周边买入':
            wif.app.text('粮食交易指令')
            if queding==True:
            ...
            wif.app.text('粮食交易指令')
            if queding==True:
                sjk['AI'].zl(dqid=dqid,zl='粮食交易',cs=[mbdqid,wif.appuifw.query(cn("粮食交易本地卖出（0-%s）单位"%(dqdx['粮食']['国库'])),'number'),'本地卖出'])
        elif hdlx=='粮食交易本地买入':
            wif.app.text('粮食交易指令')
            if queding==True:
...
def gjmb(chushi=None,chakan=None):
    ...
    if chakan=='地区':
        ...
        if len(gjdx['地区'])>0:
            for na in gjdx['地区']:
                na=sjk['地区'][na]
                tab.row([('text','%s(id:%s)'%(na['地区名'],na['id'])),('text','(x:%s,y:%s)'%na['坐标']),('link','查看大地图',lambda:[wif.funcsx(None),wif.hide(),ddt(gcwz=na['坐标'])])])
        else:
