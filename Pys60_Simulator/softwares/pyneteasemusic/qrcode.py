...
# e.g. rsPoly = base.Polynomial(rsPoly_LUT[ecCount], 0)
rsPoly_LUT = {
...
def funcrs_blocks(version, error_correction):
...
BIT_LIMIT_TABLE = [
    [0] + [8*sum(map(_data_count, funcrs_blocks(version, error_correction)))
...
def optimal_data_chunks(data, minimum=4):
    ...
    if len(data) <= minimum:
        ...
        re_repeat = (
            b('{') + text_type(minimum).encode('ascii') + b(',}'))
...
def create_data(version, error_correction, data_list):
    ...
    rs_blocks = funcrs_blocks(version, error_correction)
    bit_limit = 0
...
class PilImage(BaseImage):
    ...
    def new_image(self, **kwargs):
        ...
        if fill_color.lower() != "black" or back_color.lower() != "white":
            ...
            if back_color.lower() == "white": back_color = (255,255,255)

        img = Image.new((self.pixel_size, self.pixel_size),mode)
        img.clear(back_color)
...
class QRCode:
    ...
    def best_mask_pattern(self):
        ...
        for i in range(8):
            ...
            lost_point = funclost_point(self.modules)
    ...
    def map_data(self, data, mask_pattern):
        ...
        mask_func = funcmask_func(mask_pattern)
