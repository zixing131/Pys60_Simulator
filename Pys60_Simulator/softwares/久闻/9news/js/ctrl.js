...
const info = { //应用属性
	appname: '久闻s60v3客户端',
	version: '0.0.2',
	summary: '“久闻”是久智网旗下的一款小众系统科技资讯客户端。我们励志为小众系统用户提供最新最全的科技资讯。',
	thanks: '感谢Q君裸奔群的小伙伴们，以及久智网一直支持我们的网友们~',
	website: 'www.9smart.cn'
};
const CMD = { //菜单执行的命令
	refersh : 1, //刷新
	about: 2, //关于
...
function setTitle(str) { //设置标题栏
	getById('topic').textContent = str || '久闻-小众系统资讯';
}
function displayList(url) { //显示文章列表
	ref_page = 'list';
	ajax_get('http://api2.9smart.cn/articles' + (url?'/'+url:''), function(error, data) {
		if (error) {
			alert(error);
		} else {
			const articles = [];
			data.articles.forEach(function(item) {
				articles.push('<li><a href="javascript:displayArticle(\''+item._id+'\')">'+(item.attachments.length?'<img src="'+item.attachments[0].thumb+'" alt="截图" />':'')+'<div><h3>'+item.topic+'</h3><p><span>评论('+item.comments+')</span><span>人气('+item.views+')</span></p></div></a></li>');
			});
...
function displayArticle(aid) { //显示文章正文
	ref_page = 'article';
	list_top = document.body.scrollTop;
	ajax_get('http://api2.9smart.cn/article/'+aid, function(error, data) {
		if (error) {
			alert(error);
		} else {
			setTitle(data.article.topic);
			getById('dateline').textContent = dateline(data.article.dateline);
			getById('author').textContent = data.article.author.nickname;
			getById('comment_num').textContent = '评论('+data.article.comments+')';
			getById('content').innerHTML = UBB(data.article.content);
			getById('content').className = aid;
			showPage('article');
		}
	});
}
