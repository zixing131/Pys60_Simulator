...
# Desktop changes: Python 3 syntax, SQLite backend, stdlib exports, undated-todo persistence.
import _s60calendar as _calendar
...
class CalendarDb(object):
    ...
    def add_appointment(self):
        return CalendarDb.AppointmentEntry(
            self._db.add_entry(_calendar.entry_type_appt), self, locked="as_new_entry"
        )

    def add_event(self):
        return CalendarDb.EventEntry(
            self._db.add_entry(_calendar.entry_type_event), self, locked="as_new_entry"
        )

    def add_anniversary(self):
        return CalendarDb.AnniversaryEntry(
            self._db.add_entry(_calendar.entry_type_anniv), self, locked="as_new_entry"
        )

    def add_reminder(self):
        return CalendarDb.ReminderEntry(
            self._db.add_entry(_calendar.entry_type_reminder),
            self,
            locked="as_new_entry",
        )

    def add_todo(self):
        return CalendarDb.TodoEntry(
            self._db.add_entry(_calendar.entry_type_todo), self, locked="as_new_entry"
        )
    ...
    def monthly_instances(
        self, month, appointments=0, events=0, anniversaries=0, todos=0, reminders=0
    ):
        return self._db.monthly_instances(
            month,
            self._create_filter(appointments, events, anniversaries, todos, reminders),
        )

    def daily_instances(
        self, day, appointments=0, events=0, anniversaries=0, todos=0, reminders=0
    ):
        return self._db.daily_instances(
            day,
            self._create_filter(appointments, events, anniversaries, todos, reminders),
        )
    ...
    def find_instances(
        ...
        return self._db.find_instances(
            start_date,
            end_date,
            str(search_string),
            self._create_filter(appointments, events, anniversaries, todos, reminders),
    ...
    class Entry(object):
        ...
        def _set_content(self, content):
            self._fetch_entry()
            self._entry.set_content(str(content))
            self._autocommit()
        ...
        def _set_description(self, description):
            self._fetch_entry()
            self._entry.set_description(str(description))
            self._autocommit()
        ...
        def _set_location(self, location):
            self._fetch_entry()
            self._entry.set_location(str(location))
            self._autocommit()
        ...
        def _replication(self):
            if self._entry.replication() in _replicationreversemap:
