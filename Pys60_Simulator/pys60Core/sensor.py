...
class OrientationCalibration:
    def __init__(self):
        self.timeout = 0.1
        self.gravity = (
            300  # approx value of an acc sensor when directly exposed to gravity
        )
        #
        ...
        #                     X    Y    Z
        self.ival_span = [
            [100, 200, 300],  # vertical (top, bottom)
...
class OrientationEventFilter(EventFilter):
    ...
    def event(self, sensor_val):
        ...
        if (
            _in_ival(x, 0, orientation_calibration.ival_span[0][0])
            and _in_ival(
                y,
                -orientation_calibration.gravity,
                orientation_calibration.ival_span[0][1],
            )
            and _in_ival(z, 0, orientation_calibration.ival_span[0][2])
        ):
            orientation_tmp = orientation.TOP
            ...
            _in_ival(
                x,
                -orientation_calibration.gravity,
                orientation_calibration.ival_span[1][0],
            )
            and _in_ival(y, 0, orientation_calibration.ival_span[1][1])
            and _in_ival(z, 0, orientation_calibration.ival_span[1][2])
        ):
            ...
            _in_ival(x, 0, orientation_calibration.ival_span[2][0])
            and _in_ival(y, 0, orientation_calibration.ival_span[2][1])
            and _in_ival(
                z,
                orientation_calibration.gravity,
                orientation_calibration.ival_span[2][2],
            )
        ):
        ...
        if (
            orientation_tmp != self.__orientation_old
        ):  # if the orientation is different than the saved one
            if (
                orientation_tmp != self.__orientation_cur
            ):  # if it has changed before we could count to orientation_calibration.max_count
